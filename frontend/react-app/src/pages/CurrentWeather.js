    ...
    const fetchWeatherData = async () => {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_BACKEND}/weather?name=${cityName}`
        );
        if (!response.ok) {
      ...
      <Card className='weather-card'>
        <Card.Body>
          <Row className='align-items-center'>
            ...
            <Col md={8}>
              <p>
                <strong>Descrição:</strong> {weatherData.weather_description}
              </p>
              <p>
              ...
              <p>
                ...
                {weatherData.feels_like.toFixed(2)}°C
              </p>
              <p>
                <strong>Umidade:</strong> {weatherData.humidity}%
              </p>
              <p>
                <strong>Nuvens:</strong> {weatherData.cloudiness}%
              </p>
            </Col>
          </Row>
        </Card.Body>
