...
async function getWeatherByCoords(lat, lon) {
    if (typeof lat !== 'number' || typeof lon !== 'number' || isNaN(lat) || isNaN(lon)) {
        throw new APIError({ status: 400, message: 'Latitude and longitude must be valid numbers' });
    }

    const city = await cityFromCoords(lat, lon);
    if (!city)
        throw new APIError({status: 404, message: 'No city found for the given coordinates'});

    return await getWeatherById(city.id);
}

module.exports = { getWeatherByCityName, getWeatherById, getWeatherByCoords };
